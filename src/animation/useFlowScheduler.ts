...
export function useFlowScheduler(
  ...
  const maxConcRef = useRef<number>(initialMaxConcurrent ?? S.maxConcurrent);
  ...
  useEffect(() => {
    ...
    maxConcRef.current = initialMaxConcurrent ?? S.maxConcurrent;

    let rampTimer: number | undefined;
    if (initialMaxConcurrent && initialMaxConcurrent < S.maxConcurrent) {
      rampTimer = window.setTimeout(() => {
        maxConcRef.current = S.maxConcurrent;
      }, rampMs ?? 1500);
      timers.current.push(rampTimer);
    }

    const startOne = () => {
      const running = Object.values(activeRef.current).filter(Boolean).length;
      if (running >= maxConcRef.current) return;
  ...
  }, [enabled, shuffled, initialMaxConcurrent, rampMs]);
  ...
  const onFlowCycleEnd = (id: string) => {
    ...
    if (S.loop) {
      ...
      window.setTimeout(() => {
        ...
        const tryStart = () => {
          const running = Object.values(activeRef.current).filter(Boolean).length;
          if (running < maxConcRef.current && queue.current.length) {
