  ...
  const items: ContactItem[] = [
    {
      ...
      caption: contact.mobileE164,
      type: 'phone',
      action: () => {
        if (isMobile) {
          window.location.href = `tel:${contact.mobileE164}`
        } else {
          handleCopy(contact.mobileDisplay, 'phone')
        }
      },
    },
    {
          ...
          {items.map((item, index) => {
            ...
            if (item.label === 'Languages' && contact.languages) {
              return (
                <div
                  key={index}
                  className="flex items-start gap-4 px-4 py-3 min-h-[56px]"
                >
            ...
            return (
              ...
              className={`flex items-center gap-4 px-4 py-3 rounded-lg transition-all duration-200 ${
                item.action ? 'cursor-pointer hover:bg-gray-50 active:bg-gray-100 hover:shadow-sm' : ''
              } min-h-[56px]`}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-base font-medium text-gray-900 truncate">{item.value}</div>
                  {item.caption && (
                    <div className="text-xs text-gray-400 mt-0.5">{item.caption}</div>
                  )}
                </div>
                {item.action && (
