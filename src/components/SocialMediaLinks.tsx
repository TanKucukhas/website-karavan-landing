...
export default function SocialMediaLinks({ variant = 'dark', className = '' }: SocialMediaLinksProps) {
  const iconClasses = variant === 'dark' 
    ? 'w-5 h-5 text-gray-400 group-hover:text-white transition-colors'
    : 'w-5 h-5 text-gray-600 group-hover:text-brand-600 transition-colors';
  
  const containerClasses = variant === 'dark'
    ? 'w-10 h-10 bg-gray-800 rounded-lg flex items-center justify-center hover:bg-brand-600 transition-colors group'
    : 'w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center hover:bg-brand-50 transition-colors group';
